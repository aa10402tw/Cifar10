...
class ResNeXt(nn.Module):
    def __init__(self, ResidualBlock, cardinality, width, num_classes=10):
        ...
        self.layer1 = self.make_layer(ResidualXtBlock, in_channels=32, width=32, out_channels=64,  num_blocks=1, stride=2)
        pytorch_total_params = sum(p.numel() for p in self.layer1.parameters() if p.requires_grad)
        print(pytorch_total_params)

        self.layer2 = self.make_layer(ResidualXtBlock, in_channels=64, width=32, out_channels=64,  num_blocks=1, stride=2) 
