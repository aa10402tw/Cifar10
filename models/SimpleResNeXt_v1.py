...
class ResNeXt(nn.Module):
    ...
    def __init__(self, ResidualBlock, cardinality, width, num_classes=10):
        ...
        self.layer1 = self.make_layer(ResidualXtBlock, in_channels=32,
                                      width=32, out_channels=64,  num_blocks=1, stride=2)
        pytorch_total_params = sum(p.numel() for p in self.layer1.parameters() if p.requires_grad)
        # print(pytorch_total_params)

        self.layer2 = self.make_layer(ResidualXtBlock, in_channels=64,
                                      width=32, out_channels=64,  num_blocks=1, stride=2)
    ...
    def make_layer(self, block, in_channels, width, out_channels, num_blocks, stride):
        # if num_blocks = 2, stride=2, strides = [2,1] (reduce dim at first block)
...
def SimpleResNeXt_v1():
    return ResNeXt(ResidualXtBlock, cardinality=32, width=128)
...
if __name__ == '__main__':
    # test_ResNeXt()
    pytorch_total_params = sum(p.numel() for p in nn.BatchNorm2d(1).parameters() if p.requires_grad)
    for p in nn.BatchNorm2d(1).parameters():
        print(p)
    print(pytorch_total_params)
